...
import { createBooking, cancelBooking } from "./bookingController"; // ✅ Correct import
...
export const initiatePayment = asyncHandler(async (req: any, res: Response) => {
  ...
  if (!salonService) return res.status(404).json({ error: "Service not found" });
...
export const mpesaCallback = asyncHandler(async (req: any, res: Response) => {
  const { Body } = req.body;
  if (!Body || !Body.stkCallback) return res.status(400).json({ error: "Invalid callback data" });
...
// ======================================================================================
async function cancelBookingDirect(bookingId: string) {
  await prisma.booking.update({
    where: { id: bookingId },
